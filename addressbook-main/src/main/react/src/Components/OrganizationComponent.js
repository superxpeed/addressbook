...
export class OrganizationComponent extends React.Component {
    ...
    getValidationState(field) {
        if (
            this.state.organization[field] === undefined ||
            this.state.organization[field] === null
        )
            return "error";
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.organization !== this.state.organization) {
            if (
                this.state.organization["id"] !== undefined &&
    ...
    getButton() {
        if (this.state.create) {
            return (
                <Button style={{width: "100%"}} onClick={this.saveOrganization}>
                    Create organization
                </Button>
            );
        } else {
