...
@connect(
  (state) => ({
    ...
    fieldDescriptionMapOrganization:
      state.universalListReducer.fieldDescriptionMapOrganization,
    totalDataSizeOrganization:
      state.universalListReducer.totalDataSizeOrganization,
    tableDataPerson: state.universalListReducer.tableDataPerson,
    fieldDescriptionMapPerson:
      state.universalListReducer.fieldDescriptionMapPerson,
    ...
    selectedRowsOrganization:
      state.universalListReducer.selectedRowsOrganization,
  ...
  (dispatch) => ({
    ...
    clearPersonSelection: bindActionCreators(
      CommonActions.clearPersonSelection,
      dispatch
    ),
...
export default class UniversalListForm extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.selectedRowsOrganization.length === 1 &&
      (nextProps.selectedRowsOrganization.length !==
        this.props.selectedRowsOrganization.length ||
        nextProps.selectedRowsOrganization[0].id !==
          this.props.selectedRowsOrganization[0].id)
    ) {
      let converted = [];
      converted.push({
        name: "orgId",
    ...
    if (
      nextProps.selectedRowsOrganization.length === 0 &&
  ...
  refreshTable = (start, pageSize, sortName, sortOrder, filterDto, cache) => {
    ...
    this.props.getList(
      Url.GET_LIST_4_UNIVERSAL_LIST_FORM +
        "?start=" +
        start +
        "&pageSize=" +
        pageSize +
          ...
          <Tab key={1} eventKey={1} title="Organizations">
            <div style={{ marginBottom: "15px" }}>
              <OrganizationComponent
                organization={
                  this.props.selectedRowsOrganization.length === 1
                    ? this.props.selectedRowsOrganization[0]
                    : {}
                }
              />
            </div>
